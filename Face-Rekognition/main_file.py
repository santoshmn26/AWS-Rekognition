...
def attempt_login(login_attempt):
    ...
    # i=counter to increase line animation
    #r = radius of arc
    #j = counder to decrease r
    i,r,j=1,30,1                         
        ...
        if ret:
            ...
            if(len(faces)):             # True if a face is detected in the frame
                if(found_face==0):      # Complete face not scanned yet    
                    # Check if the face present is in the center of the frame
                    dist = math.hypot(faces[0][0]-275, faces[0][1]-175)
